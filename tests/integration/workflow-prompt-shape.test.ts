...
describe("workflow prompt shape", () => {
  ...
  it("guides runtime ontology discovery, type selection, and relation building", () => {
    ...
    expect(prompt).toContain("Discover the current page type ontology via");
    expect(prompt).toContain("## Step 2 — Decide");
    expect(prompt).toContain("### 2b. Type Selection");
    expect(prompt).toContain("### 2d. Page Granularity and Splitting");
    expect(prompt).toContain("At most 5 pages per vault source");
    expect(prompt).toContain("### 2e. Building Relations");
    ...
    expect(prompt).toContain("**relatedPages**: Thematic relations only");
